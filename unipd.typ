...
#let title-slide(
    ...
    if type(subtitle) == none { subtitle = "" }
    if type(authors) != array { authors = (authors,) }
    if type(date) == none { date = "" }

    v(top-gap)
    align(center, box(inset: (x: 2em), text(size: 39pt, fill: white, title)))
    v(title-subtitle-gap)
    align(center, box(inset: (x: 2em), text(size: 28pt, fill: white, subtitle)))
    v(meta-gap)
